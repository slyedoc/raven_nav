...
impl NavigationMesh {
    ...
    pub fn validate(mut self) -> Result<ValidNavigationMesh, ValidationError> {
        ...
        let mesh_bounds = vertices.iter().fold(BoundingBox::Empty, |acc, &vertex| {
        ...
        for (polygon_index, polygon) in self.polygons.iter().enumerate() {
            ...
            for i in 0..polygon.len() {
                let left_vertex = polygon[if i == 0 { polygon.len() - 1 } else { i - 1 }];
                let center_vertex = polygon[i];
                let right_vertex = polygon[if i == polygon.len() - 1 { 0 } else { i + 1 }];
